// export const PRIMARY = '#2FB144';
export const PRIMARY = '#FF7764';
// export const PRIMARY_STRONG = '#27973b';
export const PRIMARY_STRONG = '#ff513e';
export const PRIMARY_LIGHT = '#fff2f0';
export const SECONDARY = '#1E1F28';
export const THIRD = '#F8F8F8';
export const WHITE = '#FFFFFF';
export const WHITE_LIGHT = '#fafafc';
export const WHITE_MEDIUM = '#f7f6fb';
export const WHITE_THIN = '#eaebf2';
export const FORM_BG = '#f3f4f8';
export const BLUE = '#1778F2';
export const BLACK = '#000000';
export const DARK = '#312651';
export const DARK_SECOND = '#1C1C1C';
export const DARK_SECONDARY = '#1C1A32';
export const TEXT_INPUT_COLOR = '#1C1A32';
// export const TEXT_INPUT_COLOR_GRAY = '#F9F9F9';
export const TEXT_INPUT_COLOR_GRAY = '#e6e6e6';
export const TEXT_PRIMARY = '#83829A';

export const GHJOB_LOGO_WHITE = '#eff6fe';
export const GHJOB_LOGO_GRAY = '#d9e4ef';

// ACTIONS
export const SUCCESS = '#3adb76';
export const WARNING = '#ffae00';
export const ALERT = '#cc4b37';

// GRAYSCALE
export const GRAY_LIGHT = '#E6E4E6';
export const GRAY_MEDIUM = '#cacaca';
export const GRAY_DARK = '#83829A';

export const BOND_BLUE = '#4267B2';
